...
def only_commenter(func):
    ...
    def check(*args,**kwargs):
        comment = db.session.execute(db.select(Comment).where(Comment.id == kwargs["comment_id"])).scalar()
        if not current_user.is_authenticated:
            return abort(403)
        if comment is None:
            return abort(404)
        elif current_user.id == comment.author_id or current_user.id == 1 :
            return func(*args,**kwargs)
        else:
            return abort(403)
    return check
...
def gravatar_url(email,size=100,rating='g',default='retro'):
    ...
    return f"https://www.gravatar.com/avatar/{hash_value}?={size}&d={default}&r={rating}"
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if check_email:
            flash("You already signed with that email,log in instead!")
